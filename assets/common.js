...
kakaoButton&&kakaoButton.addEventListener('click',async function(){if(window.Kakao&&window.Kakao.Share){window.Kakao.Share.sendDefault({objectType:'feed',content:{title:shareTitle,description:kakaoDescription,imageUrl:imageUrl,imageWidth:1200,imageHeight:630,link:{mobileWebUrl:pageUrl,webUrl:pageUrl}},buttons:[{title:'FIND MY NEXT STATION',link:{mobileWebUrl:pageUrl,webUrl:pageUrl}}]});return;}await copyUrlWithMessage('카카오톡 공유를 불러오지 못했습니다. 카카오톡에 붙여넣어 공유해 주세요.');});
